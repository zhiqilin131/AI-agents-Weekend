"""Pre-run clarification gate — memory-aware LLM only (no generic templates)."""
...
def run_clarify_gate(
    ...
    *,
    ...
    """Memory-first clarify: fast heuristics → memory sufficiency → personalized LLM (no template fallback)."""
    text = raw.strip()
    ...
    if sufficient:
        timing["clarification_suppressed_reason"] = f"memory_sufficient:{suff_reason}"
        return ClarifyGateResult(
            need_clarification=False,
            skip_reason="not_needed",
            clarification_meta=timing,
    ...
    if llm is None:
        timing["clarification_suppressed_reason"] = "memory_gated_no_llm"
        return ClarifyGateResult(
            need_clarification=False,
            skip_reason="no_llm",
            clarification_meta=timing,
    ...
    llm_out, llm_ms = run_personalized_clarify_gate_timed(
        text,
        llm,
        timeout_s=CLARIFY_LLM_TIMEOUT_S,
        profile=prof,
    ...
    if llm_out is not None:
        ...
        if llm_out.need_clarification and llm_out.questions:
            ...
            merged_meta["why_this_question"] = merged_meta.get("why_this_question") or (
                "Memory and your message still leave a gap — this choice fills it."
            )
            return ClarifyGateResult(
        ...
        merged_meta["clarification_used_llm"] = bool(llm_ms is not None)
    ...
    timing["clarification_suppressed_reason"] = "memory_gated_llm_timeout"
    return ClarifyGateResult(
        need_clarification=False,
        skip_reason="error",
        clarification_meta=timing,
