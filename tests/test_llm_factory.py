...
from foresight_x.orchestration.llm_gateway import LLMGateway
...
def test_build_openai_llm_routes_gpt5_to_responses(monkeypatch, tmp_path) -> None:
    ...
    raw = llm.primary_client
    assert isinstance(raw, OpenAIResponses)  # compat subclass
    assert raw.model == "gpt-5.4-mini"
    assert raw.temperature == pytest.approx(0.3)
    assert raw.additional_kwargs.get("reasoning") == {"effort": "low"}
...
def test_build_openai_llm_maps_max_tokens_for_responses(monkeypatch, tmp_path) -> None:
    ...
    raw = llm.primary_client
    assert isinstance(raw, OpenAIResponses)
    assert raw.max_output_tokens == 4096
