"""Replay quality E2E scenarios (fictional personas, separate from tests/eval)."""
    ...
    *,
    ...
    try:
        trace = run_pipeline(
            ctx,
            user_input,
            decision_id=f"quality-{scenario.id}-t{turn_index}",
            persist_trace=False,
        )
...
def replay_quality_scenario(scenario: QualityE2EScenario, *, model_id: str | None = None) -> list[TurnResult]:
    ...
    if isinstance(scenario.input, str):
        ...
        return [
            _run_shadow_turn_quality(
                scenario=scenario,
                messages=msgs,
                turn_index=0,
                thread_id=f"quality-{scenario.id}",
                model_id=active_model,
            )
        ]
    ...
    for idx, user_text in enumerate(turns):
        ...
        result = _run_shadow_turn_quality(
            scenario=scenario,
